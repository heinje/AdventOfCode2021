using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;

namespace AoC01
{
    class Program
    {
        static void Main(string[] args)
        {
            var filename = "Input.txt";
            task1(filename);
            task2(filename);
        }

        private static void task1(string filename)
        {
            Console.WriteLine("Task 1:");
            Console.WriteLine("=======");
            int oldValue = int.MaxValue;
            int timesIncreased = 0;
            foreach (var value in readNumbersFromFile(filename))
            {
                if (value > oldValue)
                {
                    timesIncreased++;
                }

                oldValue = value;
            }

            Console.WriteLine($"{timesIncreased} increases");
            Console.WriteLine();
        }

        private static void task2(string filename)
        {
            Console.WriteLine("Task 2:");
            Console.WriteLine("=======");
            int oldValue = int.MaxValue;
            int timesIncreased = 0;
            var values = readNumbersFromFile(filename).ToArray();
            for (var i = 2; i < values.Length; i++)
            {
                var newValue = values[i - 2] + values[i - 1] + values[i];
                if (newValue > oldValue)
                {
                    timesIncreased++;
                }

                oldValue = newValue;
            }

            Console.WriteLine($"{timesIncreased} increases");
            Console.WriteLine();
        }

        private static IEnumerable<int> readNumbersFromFile(string fileName)
        {
            var inputFile = new FileInfo(fileName);
            using var fileStream = inputFile.OpenRead();
            using var fileReader = new StreamReader(fileStream);
            while (!fileReader.EndOfStream)
            {
                var number = fileReader.ReadLine();
                if (int.TryParse(number, out var value))
                {
                    yield return value;
                }
            }
        }
    }
}
